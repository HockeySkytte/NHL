...
def _load_card_metrics_defs() -> Dict[str, Any]:
    ...
    try:
        ...
        ttl_s = 600
    now = time.time()
    if _CARD_METRICS_DEF_CACHE and (now - _CARD_METRICS_DEF_CACHE[0]) < ttl_s:
        return _CARD_METRICS_DEF_CACHE[1]
    ...
    try:
        ...
        with open(path, 'r', encoding='utf-8-sig', newline='') as f:
            ...
            for row in rdr:
                ...
                if card and card.lower() != 'skaters':
                    continue
...
def api_player_projections_league():
    """League-wide skater projections.
        ...
        try:
            ...
            out.append({
                **row,
                ...
                'position': pos if pos in ('F', 'D') else (info.get('position') or 'F'),
            })
        except Exception:
            continue
