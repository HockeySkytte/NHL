...
Defaults:
- Table: nhl_seasonstats_teams
- Output: app/static/nhl_seasonstats_teams.csv
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export nhl_seasonstats_teams from MySQL to app/static CSV")
    parser.add_argument("--mode", choices=["ro", "rw"], default="ro", help="Which DB URL to use")
    parser.add_argument(
        "--db-url",
        default="",
        help=(
            "Optional explicit SQLAlchemy DB URL (overrides env vars). "
            "Example: mysql+mysqlconnector://user:pass@host:3306/public"
        ),
    )
    parser.add_argument("--table", default="nhl_seasonstats_teams", help="Table name to export")
    parser.add_argument(
        "--out",
        default=str(Path(__file__).resolve().parents[1] / "app" / "static" / "nhl_seasonstats_teams.csv"),
